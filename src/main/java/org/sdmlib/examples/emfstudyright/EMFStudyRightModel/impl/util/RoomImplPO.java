...
public class RoomImplPO extends PatternObject<RoomImplPO, RoomImpl>
{
   ...
   {
      return this.startCreate().hasDoorsRev().endCreate();
   }

   public RoomImplPO hasDoorsRev(RoomPO tgt)
   {
      return hasLinkConstraint(tgt, Room.PROPERTY_DOORSREV);
   }

   public RoomImplPO createDoorsRev(RoomPO tgt)
   {
      return this.startCreate().hasDoorsRev(tgt).endCreate();
   }
   ...
   public RoomSet getDoorsRev()
   {
      if (this.getPattern().getHasMatch())
      {
         return ((Room) this.getCurrentMatch()).getDoorsRevSet();
      }
      return null;
   }
